...
#----Read the dataset------
getwd()
setwd("C:/Users/Raj/Documents/R/Matrix Data structure")
...
library(sqldf)
sqldf("select Home_state, COUNT (*) as obs, avg(Annual_EE_Contribution) from df GROUP BY 1")
...
sqldf("select marital_status, count(*) as obs, avg(Annual_EE_Contribution)  from df group by 1")
...
sqldf("select Employee_Division, count(*) as obs, avg(Annual_EE_Contribution)  from df  group by 1")
...
sqldf("select Age_Grp, count(*) as obs, avg(Annual_EE_Contribution)  from df  group by 1")
...
View(insurance_premium1)
#-------Regression Model--------------
reg_ins_formula <- lm(Annual_EE_Contribution ~ #State1+State2+
                        #State1+
                      ...
                      +Age_Grp2+Gender1+
                       ...
                       # Exp_Within_Org1+Exp_Within_Org2+
                        #Employee_Class1
                      +Coverage.Level1+Coverage.Level2,
